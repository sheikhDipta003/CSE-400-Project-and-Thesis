...
bucket_name = "boto3-demo-bucket-dipta"
...
def create_bucket(bucket_name, region=None):
    ...
    try:
        if region is None:
            ...
            s3_client.create_bucket(Bucket=bucket_name)     # by default, buckets are created in us-east-1 region
...
# create_bucket("eampty-bucket-example-dipta", region_name)
def delete_empty_bucket(bucket):
    s3_client = boto3.client('s3')
    response = s3_client.delete_bucket(Bucket=bucket)
    print(response)
# delete_empty_bucket("eampty-bucket-example-dipta")
